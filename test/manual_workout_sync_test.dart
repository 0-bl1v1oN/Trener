  ...
  group('manual workout sync', () {
    ...
    test(
      ...
      () async {
        ...
        expect(client['uuid'], selected.client.externalId);
        ...
        expect(workout['uuid'], selected.workoutExternalId);
        expect(workout['performed_at'], isNotNull);
        expect(workout['day_index'], selected.template.idx);
        expect(workout['day_label'], selected.template.label);
        expect(workout['day_title'], selected.template.title);
        expect(workout['plan_instance'], 1);
        expect(exercise['uuid'], selected.exerciseExternalId);
    ...
    test('HTTP error keeps both tasks and never starts the next one', () async {
      final db = AppDb.forTesting(NativeDatabase.memory());
      addTearDown(db.close);
      final selected = await _createWorkout(
        db,
